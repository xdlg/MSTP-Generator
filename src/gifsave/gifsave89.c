 ...
 *		pixcheck(ncols,nrows,pixels,nchk,ncolors,bg,fg1,fg2) chker bg
 *		+---
 ...
 * check input
 * -------------- */
if ( gs == NULL ) goto end_of_job;	/* failed to malloc gif data struct*/
width     = min2(9999,max2(1,width));	/* 1 <= width  <= 9999 */
height    = min2(9999,max2(1,height));	/* 1 <= height <= 9999 */
gs->width = width; gs->height = height;	/* reset values stored in gs */
...
if ( version == 87 ) memcpy(Version,"87a",4); /* reset to 87a if it is 87 */
if ( !OKAY ) goto end_of_job;		/* quit if input check failed */
/* ---
...
if ( OKAY ) status = putblkbytes(bk,Version,3);
/* --- Logical Screen Descriptor --- */
if ( OKAY ) status = putblkword(bk,width);
if ( OKAY ) status = putblkword(bk,height);
if ( OKAY ) status = putblkbyte(bk,Packed);
if ( OKAY ) status = putblkbyte(bk,bgindex);
if ( OKAY ) status = putblkbyte(bk,AspectRatio);
if ( !OKAY ) goto end_of_job;
/* ---
 * global color table
 * --------------------- */
...
if ( ncolorbits > 0 ) {			/* caller provided global colors[] */
  putgifcolortable(gs,colors); }	/* emit global color table */
end_of_job:
  if ( !OKAY )				/* something fouled up */
    if ( gs != NULL ) {			/* but we have an allocated gs */
      free((void *)gs); gs=NULL; }	/* free it and signal error */
 ...
 * check input
 * -------------- */
if ( bk == NULL ) goto end_of_job;	/* no gif data struct supplied */
if ( gs->version == 87 ) goto end_of_job; /* no extensions for GIF87a */
if ( gs->isanimated > 0 ) goto end_of_job; /* just one animategif() per gif*/
if ( nrepetitions<0 || nrepetitions>65535 ) nrepetitions=0; /*0=continuous*/
/* ---
...
if ( OKAY ) status = putblkbyte(bk,3);	/* 3 more bytes of data */
if ( OKAY ) status = putblkbyte(bk,1);	/* data sub-block index (always 1) */
if ( OKAY ) status = putblkword(bk,nrepetitions); /* #repetitions */
if ( OKAY ) status = putblkbyte(bk,0);	/* sub-block sequence terminator */
if ( OKAY ) {
  ...
  gs->isanimated++; }			/* set isanimated flag true */
end_of_job:
  return ( status );
 ...
 * check input
 * -------------- */
if ( bk==NULL || sb==NULL ) goto end_of_job; /*no gif data struct supplied*/
if ( gs->version == 87 ) goto end_of_job; /* no extensions for GIF87a */
cellwidth = (nchars<1?0:width/nchars);	/* #pixels/char width */
if ( !ismimetex ) {			/* irrelevant args if using mimetex*/
  if ( cellwidth<2 || height<2 ) goto end_of_job; /* won't work */
  if ( nchars < 1 ) goto end_of_job; }	/* no data won't work, either */
/* ---
...
if ( ismimetex )			/* but for mimeTeX... */
 if ( nchars < 1 ) {			/* ...no data signals reset */
  ...
  if ( gs->pt_pixels != NULL ) {	/* have previous allocated image */
    free(gs->pt_pixels);  gs->pt_pixels=NULL; } /* free it and reset ptr */
...
if ( ismimetex ) {			/* use mimeTeX to render plaintext */
  ...
  if ( width<0 || height<0 ) {		/* set persistent text flag */
    ...
    width=absval(width); height=absval(height); } /* reset neg signal */
  gs->pt_left=left;  gs->pt_top=top;	/* save top-left corner col;row */
  gs->pt_bg  =bg;    gs->pt_fg =fg;	/* save bg,fg colortable indexes */
  gs->pt_width = gs->pt_height = 0;	/*reset pixelized text width,height*/
...
if ( !ismimetex ) {			/* emit gif89a plaintent extension */
  ...
  if ( OKAY ) status = putblkbyte(bk,Label);
  if ( OKAY ) status = putblkbyte(bk,BlockSize);
  if ( OKAY ) status = putblkword(bk,left);
  if ( OKAY ) status = putblkword(bk,top);
  if ( OKAY ) status = putblkword(bk,width);
  if ( OKAY ) status = putblkword(bk,height);
  if ( OKAY ) status = putblkbyte(bk,cellwidth);
  if ( OKAY ) status = putblkbyte(bk,cellheight);
  if ( OKAY ) status = putblkbyte(bk,fg);
  if ( OKAY ) status = putblkbyte(bk,bg);
  if ( OKAY ) status = putsubbytes(sb,(BYTE *)data,nchars);
  if ( OKAY ) status = flushsubblock(sb);
  if ( OKAY ) status = putblkbyte(bk,Terminator);
  } /* --- end-of-if(!ismimetex) --- */
end_of_job:
  return ( status );
 ...
 * entry point
 * -------------- */
int	controlgif ( GS *gs, int tcolor, int delay,
	int userinput, int disposal )
{
/* --- allocations and declarations --- */
int	status  = (-1);			/* init signalling error */
int	Introducer = 0x21;		/* always 0x21 */
...
if ( bk == NULL ) goto end_of_job;	/* no gif data struct supplied */
if ( gs->version == 87 ) goto end_of_job; /* no extensions for GIF87a */
if ( gs->ncontrol > 0 ) goto end_of_job; /* just one control per image */
if ( tcolor<0 || tcolor>255 ) tcolor=(-1); /* not transparent if illegal */
if ( delay<0 || delay>65535 ) delay=0;	/* 0 if illegal (<0 or >2**16-1) */
userinput = (userinput<=0?0:1);		/* "yes" if positive */
if ( disposal!=1 && disposal!=2		/* 0,1,2,3 are okay */
&&   disposal!=3 ) disposal = 0;	/* default to 0 otherwise */
/* ---
 * construct graphic control extension and emit it
 * -------------------------------------------------- */
...
if ( OKAY ) status = putblkbyte(bk,BlockSize);
if ( OKAY ) status = putblkbyte(bk,Packed);
if ( OKAY ) status = putblkword(bk,delay);
if ( OKAY ) status = putblkbyte(bk,(tcolor<0?0:tcolor));
if ( OKAY ) status = putblkbyte(bk,Terminator);
if ( OKAY ) gs->ncontrol++;		/* count (another?) graphic control*/
end_of_job:
  return ( status );
 ...
 * check input
 * ----------- */
if ( bk == NULL ) goto end_of_job;	/* no gs data provided */
...
if ( gs->isanimated > 0 )		/* rendering gif animation */
  if ( gs->ncontrol < 1 ) {		/* but no graphic control for frame*/
    ...
    if ( !OKAY ) goto end_of_job;	/* quit if failed */
    gs->ncontrol++; }			/* count controlgif() call */
/* ---
 * image descriptor
 * ------------------- */
if ( width>=1 && height>=1 ) {
  ...
  if ( ncolorbits > 0 ) {		/* caller provided local colors[] */
    ...
    gs->ncolorbits = ncolorbits; }	/* and set #bits for local table */
  else gs->ncolorbits = gs->ncolorbits_gct; /*else default to global table*/
...
if ( pixels!=NULL && npixels>0 ) {	/* have pixels to be encoded */
  ...
  if ( gs->isplaintext > 0 ) {		/* have plaintext data */
    if ( gs->pt_pixels == NULL )	/* but need new pixel image */
      gs->pt_pixels= plainmimetext(gs->pt_data,&gs->pt_width,&gs->pt_height);
    if ( gs->pt_pixels != NULL )	/* got new image or still have old */
      overpix = overlay(pixels, gs->width,   gs->height,  /* so overlay it */
                 gs->pt_pixels, gs->pt_width,gs->pt_height,
                 gs->pt_left,gs->pt_top, gs->pt_bg,gs->pt_fg);
  ...
  if ( overpix != NULL ) free((void *)overpix); /*free text overlay pixels*/
  if ( gs->isplaintext == 1 ) {		/*plaintext was for this frame only*/
    if ( gs->pt_pixels != NULL ) {	/*should have allocated pixel image*/
      ...
      gs->pt_pixels = NULL; }		/* ...and reset its ptr */
    gs->isplaintext = 0; }		/* always reset plaintext flag */
...
if ( bk != NULL ) {			/* have buffers */
  ...
  if ( OKAY ) status = putblkbyte(bk,Trailer); /* write 0x3B trailer byte */
  if ( OKAY ) nblkbytes = bk->nblkbytes; } /*return tot #bytes in gifimage*/
if ( gs != NULL ) {			/* have gs */
...
||   colors==NULL ) goto end_of_job;	/* or no colors */
...
!= NULL ) {				/* check if init succeeded */
  ...
  ngifbytes = endgif(gs); }		/* all done */
else {					/*newgif() failed to init gifsave89*/
  if ( gifimage != NULL ) free(gifimage); /* free gifimage if malloc'ed */
  gifimage = NULL; }			/* reset returned ptr */
end_of_job:
...
if ( gs == NULL ) goto end_of_job;	/* failed to allocate memory */
...
if ( msglevel>1 && *msgfile!='\000' )	/* redirect debug output to file */
  ...
  == NULL ) gs->msgfp = MSGFP;		/* back to default if fopen fails */
...
if ( dbfile != NULL )			/* have dbfile arg */
 if ( *dbfile != '\000' ) {		/* that's not an empty string */
  ...
  msgfile[127] = '\000'; }		/* make sure it's null-terminated */
return ( 1 );
...
-------------------------------------------------------------------------- */
/* --- redirect null fp --- */
if ( fp == NULL ) fp = MSGFP;		/* default fp if null */
/* --- adjust display for hexdump --- */
if ( format != 1 ) display_width /= 3;	/* "xx xx xx..." for hex display */
/* --- header line --- */
...
while ( (locol=hicol+1) < gs->width )	/*start where prev segment left off*/
  ...
  if (hicol >= gs->width) hicol = gs->width - 1; /*but not more than pixels*/
  ...
  for ( irow=0; irow<gs->height; irow++ )  /* all scan lines for col range */
    ...
    for ( ipix=0; ipix<scan_width; ipix++ ) { /* set each char or hexbyte */
      ...
      if ( format == 1 ) scanline[ipix] = (isbg?bgchar:fgchar);
      else {				/* dump hex byte */
        int icol=3*ipix;		/* first col for display */
        if ( isbg ) scanline[icol] = scanline[icol+1] = bgchar;
        else { scanline[icol] = hexchars[(pixel/16)%16];
 ...
 * ----------------- */
/* --- check arguments --- */
if ( width  != NULL ) *width  = 0;	/* init for error */
if ( height != NULL ) *height = 0;	/* init for error */
if (  expression ==  NULL  ) goto end_of_job; /* nothing to pixelize */
if ( *expression == '\000' ) goto end_of_job; /* still nothing to pixelize */
if ( strlen(expression) > 1536 ) goto end_of_job; /*too much (sanity check)*/
/* ---
 * open pipe to wget
 * -------------------- */
...
while ( 1 ) {				/* read lines (shouldn't find eof) */
  if ( fgets(pipeline,511,wget) == NULL ) goto end_of_job; /*premature eof*/
...
if ( npixels<1 || npixels>999999 ) goto end_of_job; /* sanity check */
...
while ( 1 ) {				/* read lines (shouldn't find eof) */
  ...
  while ( ipixel < npixels ) {
    while ( *pipeptr != '\000' )	/* skip leading non-digits */
      if ( isdigit(*pipeptr) ) break;	/* done at first digit */
      else pipeptr++;			/* or skip leading non-digit */
    if ( *pipeptr == '\000' ) break;	/* no more numbers on this line */
...
if ( width  != NULL ) *width  = ncols;	/* supply width to caller */
if ( height != NULL ) *height = nrows;	/* supply height to caller */
...
BYTE	*pix = (npixels<1||npixels>999999?NULL:(BYTE *)malloc(npixels));
int	col=0, row=0;			/* pix2 col,row indexes */
/* ---
 * initialization
 * ----------------- */
/* --- check args --- */
if ( pix == NULL ) goto end_of_job;	/* bad args or malloc failed */
/* --- init pix = pix1 --- */
memcpy((void *)pix,(void *)pix1,npixels); /* pix = pix1 to begin with */
/* --- centering requested? --- */
if ( col1 < 0 ) col1 = max2(0,(w1-w2+1)/2); /* horizontal centering */
if ( row1 < 0 ) row1 = max2(0,(h1-h2+1)/2); /*  vertical centering  */
/* --- check right, bottom edges --- */
if ( col1+w2 > w1 ) col1 = max2(0,w1-w2); /* text went past right of image */
if ( row1+h2 > h1 ) row1 = max2(0,h1-h2); /* text went below bottom */
/* ---
 * now overlay pix2
 * ------------------- */
for ( row=0; row<h2; row++ ) {		/* loop over each pix2 row */
  ...
  if ( row1+row >= h1 ) break;		/* past bottom edge of pix1 */
  for ( col=0; col<w2; col++ ) {	/* and over each col in row */
    if ( col1+col >= w1 ) break;	/* past right edge of pix1 */
...
if ( colors != NULL ) {			/* caller supplied r,g,b colors[] */
  ...
  while ( colors[ncolors] >= 0 ) ncolors++; /* count r,g,b, r,g,b, ..., -1 */
  ...
  if ( ncolors > 1 ) {			/* at least 2 colors */
    while ( ncolors >= pow2 ) {		/* power of 2 strictly > ncolors */
      pow2 *= 2;  ncolorbits++; }	/* bump power of 2 and #bits */
    ncolors = pow2/2; }			/* scale back ncolors */
...
if ( gs != NULL )			/* caller wants color table written*/
 if ( ncolors > 1 ) {			/* and provided colors[] to write */
  ...
  for ( irgb=0; irgb<3*ncolors; irgb++ ) { /* each component of each color */
    if ( OKAY ) status = putblkbyte(bk,colors[irgb]); /* write byte */
    else break;				/* quit if failed */
    if ( ismsgprint(gs,32) )		/* (debug) msglevel>=32 */
      if ( irgb%3 == 0 ) {		/* one color per set of 3 r,g,b */
        char newline =			/* 6 per line */
          ((irgb+3)%18==0||irgb+3==3*ncolors?'\n':' ');
        fprintf(gs->msgfp,"%3d:%2x,%2x,%2x%c",
        irgb/3,colors[irgb],colors[irgb+1],colors[irgb+2],newline); }
    } /* --- end-of-for(irgb) --- */
  if ( !OKAY ) ncolorbits = (-1);	/* signal failure */
 ...
 * ---------------------------------- */
if ( ga == NULL ) goto end_of_job;
if ( OKAY ) status = putblkbyte(bk,Introducer);
if ( OKAY ) status = putblkbyte(bk,Label);
if ( OKAY ) status = putblkbyte(bk,BlockSize);
if ( OKAY ) status = putblkbytes(bk,ga->Identifier,8);
if ( OKAY ) status = putblkbytes(bk,ga->AuthentCode,3);
if ( OKAY && 0 ) status = putblkbyte(bk,Terminator);
end_of_job:
...
int	prefix    = NEXTFIRST,
	hashindex = 0;
int	clearlzw(), putlzw(), getlzw(),
	putsubblock(), flushsubblock();
SB	*sb = (gs==NULL?NULL:&(gs->gifsubblock)); /* ptr to gif subblock */
 ...
 * check input
 * -------------- */
if ( gs == NULL ) goto end_of_job;	/* no gs data struct */
if ( data==NULL || nbytes<1 ) goto end_of_job; /* no data to encode */
/* ---
...
for ( ibyte=0; ibyte<nbytes; ibyte++ ) {
  ...
  if ( byte > maxbyte ) byte = 0;	/* default if byte too big */
  /* ---
  ...
  if ( (hashindex=getlzw(gs,prefix,byte)) != HASHFREE ) {
    ...
    continue; }				/* try adding next byte to prefix */
  /* ---
...
if ( gs->nstrings >= NSTRINGS ) {	/* too many strings in table */
  newindex = HASHFREE;  goto end_of_job; } /* signal error to caller */
 ...
 * ---------------------------------------------------------------- */
if ( index == NEXTFIRST ) return ( ((int)byte) );
/* ---
...
if ( bk==NULL || bytes==NULL || nbytes<1 ) goto end_of_job; /* check args */
/* --- realloc gifimage as necessary --- */
while ( bk->nblkbytes+nbytes > bk->maxblkbytes ) { /* more room for nbytes */
  int  newsize   = (bk->nblkbytes<1?MAXBLKBYTES:(3*(bk->maxblkbytes))/2);
  void *newblock = realloc((void *)(*(bk->block)),newsize);
  if ( newblock == NULL ) goto end_of_job; /* failed to realloc */
  *(bk->block) = (BYTE *)newblock;	/* store reallocated block ptr */
  bk->maxblkbytes = newsize; }		/* and its new larger size */
/* --- copy caller's bytes --- */
 ...
 while ( 1 ) {				/* finished after nbits */
  ...
  if ( sb->nsubbytes+indexlen >= SUBBLOCKSIZE ) /* subblock full */
    if ( flushsubblock(sb) == (-1) )	/* flush subblock, check for error */
      { status = (-1);  break; }	/* and signal error on failure */
  /* ---
   * remaining bits fit in current byte, or fill current byte and continue
   * --------------------------------------------------------------------- */
  if ( nbits < 1 ) break;		/* done after all bits exhausted */
    ...
    putbitfield(sb->subblock[sb->nsubbytes],sb->nsubbits,fieldbits,bits);
    ...
    if ( sb->nsubbits >= BITSPERBYTE ) { /*byte exhausted, move on to next*/
      sb->nsubbits = 0; sb->nsubbytes++; } /* first bit of next byte */
    status   += fieldbits; }		/* update status */
  } /* --- end-of-while(1) --- */
...
if ( nflush < 1 )			/* nothing to flush or error */
  ...
  /* --- trap internal error --- */
  if ( nflush+indexlen > SUBBLOCKSIZE ) nflush=SUBBLOCKSIZE-indexlen;
  /* --- first, emit leading byte containing number of bytes to follow --- */
  if ( putblkbyte(bk,nflush+indexlen) == (-1) ) goto end_of_job; /*failed?*/
  /* --- next, subblock index#, if desired --- */
  if ( index >= 0 ) {			/* index# desired */
     if ( ++index > 255 ) index=1;	/* bump index#, wrap to 1 after 255*/
     if ( putblkbyte(bk,index) == (-1) ) goto end_of_job; /*quit if failed*/
     sb->index = index; }		/* store bumped index# */
...
{
/* --- allocations and declarations --- */
int	maxpixels = 10000000,		/* sanity check on nrows*ncols */
	npixels = (ncols<1||nrows<1?0:min2(ncols*nrows,maxpixels));/*#bytes*/
...
if ( pixels == NULL) goto end_of_job;	/* malloc failed (or input error) */
memset(pixels,0,npixels);		/* clear pixel bytes (bg=0 assumed)*/
if ( f==NULL || n<1 ) goto end_of_job;	/* nothing to graph */
/* ---
 * set pixels representing f[]
 * ------------------------------ */
for ( col=0; col<n; col++ )
  if ( absval(f[col]) > bigf ) bigf = absval(f[col]);
for ( row=0; row<nrows; row++ ) {
  double yval = bigf*((double)((nrows/2)-row))/((double)(nrows/2));
  for ( col=0; col<ncols; col++ ) {
    int  i = (col*(n-1))/ncols;
    int  ipixel = min2(row*ncols + col,maxpixels);
    if ( yval*f[i] >= yval*yval ) pixels[ipixel]=((BYTE)1); }
  } /* --- end-of-for(row) --- */
end_of_job:
...
/* ==========================================================================
 * Function:	pixcheck ( ncols,nrows,pixels, nchk,noff, ncolors,bg,fg1,fg2 )
 * Purpose:	checkerboard bg color of pixels (bg can be any color
 *		index, not necessarily background)
 * --------------------------------------------------------------------------
 * Arguments:	rp (I)		raster * to pixels with image to checkerboard
 *		See Notes: below for further explanation of arguments
 * Arguments:	ncols (I)	int containing width of pixels (gif image)
 *		nrows (I)	int containing height of pixels (gif image)
 *		pixels (I)	BYTE * containing pixel raster as returned
 *				by pixgraph() above, or by any similar method
 *		nchk (I)	int containing #pixels per side of checker,
 *				in the form 015025 for tiles 15pixels wide
 *				by 25pixels high, i.e., 1000*width+height
 *		noff (I)	int containing tile offset
 *				in the form 005006 for tiles offset 5pixels
 *				in the x-direction and 6pixels in y, i.e.,
 *				1000*xoffset+yoffset
 *		ncolors (I)	int containing #colortable indexes,
 *				0...ncolors-1.
 *		bg (I)		int containing colortable index to be
 *				replaced by checkerboard -- this can be your
 *				bg or fg index, or any other index
 *		fg1,fg2 (I)	ints containing alternating checkerboard
 *				colortable indexes -- if either (or both) < 0,
 *				a random number 0...ncolors-1 is used,
 *				but not bg.
 * --------------------------------------------------------------------------
 * Returns:	( BYTE * )	pixels (same as input),
 *				or NULL for any error.
 * --------------------------------------------------------------------------
 * Notes:     o
 * ======================================================================= */
/* --- entry point --- */
BYTE	*pixcheck ( int ncols, int nrows, BYTE *pixels,
	int nchk, int noff, int ncolors, int bg, int fg1, int fg2 )
{
/* --- allocations and declarations --- */
int	maxpixels = 10000000;		/* sanity check on nrows*ncols */
int	nxchk=nchk/1000, nychk=nchk%1000, /* set square checkerboard tiles */
	nxoff=noff/1000, nyoff=noff%1000; /*#pixels offset in x,y-directions*/
int	irow=0, icol=0,			/* row=top->bot, col=left->right */
	icolor=1;			/* flip fg1<-->fg2 for each square */
int	randcolor();			/* random colortable index */
static	int seed = 864297531;		/* rand/srand seed for fg1,fg2<0 */
/* --- init --- */
if ( seed != 0 ) {			/* need to init rand (and nband) */
  srand(seed);				/* init with seed */
  seed = 0; }				/*don't init again unless seed reset*/
if ( nxchk == 0 ) nxchk=nychk;		/* square if user just passes nychk */
nxoff=nxoff%(2*nxchk); nyoff=nyoff%(2*nychk); /* no need for larger offsets */
/* -------------------------------------------------------------------------
loop over upper-left corner of each checkerboard tile
-------------------------------------------------------------------------- */
if ( pixels != NULL			/* make sure caller passed pixels */
&&   ncols>0 && nrows>0			/* and valid width and height */
&&   nxchk>0 && nychk>0 )		/* and non-vanishing checker tiles */
 for ( irow=(-abs(nyoff)); irow<nrows; irow+=nychk ) {
  int	jcolor = icolor;		/* current fg1<-->fg2 */
  icolor = 3-icolor;			/* flip for 1st square in next row */
  for ( icol=(-abs(nxoff)); icol<ncols; icol+=nxchk ) {
    int iy=irow, ix=icol;		/* coords within this square */
    int color = (jcolor==1?fg1:fg2);	/* color within this square */
    jcolor = 3-jcolor;			/* flips 1<-->2 for next square */
    if ( color < 0 ) while(1) {		/* random color wanted, but not bg */
      double frand = ((double)rand())/((double)RAND_MAX); /*0.0<=frand<1.0*/
      color = (int)(((double)ncolors)*frand); /* 0 <= color < ncolors */
      if ( color != bg			/* reject bg color index */
      &&   color>=0 && color<ncolors ) break; } /* double-check boundaries */
    for ( iy=irow; iy<irow+nychk && iy<nrows; iy++ )
      for ( ix=icol; ix<icol+nxchk && ix<ncols; ix++ ) {
        int  ipixel = min2(ix + ncols*iy,maxpixels);
        if ( iy<0 || ix<0 ) continue;	/* not in-bounds yet */
        if ( pixels[ipixel] == bg )	/* only checkerboard bg */
          pixels[ipixel] = color; }	/* change pixel color index */
    } /* --- end-of-for(icol) --- */
  } /* --- end-of-for(irow) --- */
return ( pixels );			/*return checkered pixels to caller*/
} /* --- end-of-function pixcheck() --- */
...
int	colortable[999]= {255,255,255, 0,0,255,/*bg(index0)=white,fg(1)=blue*/
	0,255,0, 255,0,0,		/* also colors 2=green, 3=red */
	-1};				/* end-of-colortable[] */
int	bgindex=0, fgindex=1;		/* bg/fg colortable indexes */
...
int	ischecker = -1;			/* +1/-1 to checkerboard fg/bg */
/* --- args for all tests --- */
...
-------------------------------------------------------------------------- */
/* --- data checks --- */
if ( testnum != 2 ) testnum=1;		/* input check: force test#1 or #2 */
if ( strlen(text) > 1 ) istext=1;	/* have overlay text */
...
if ( testnum == 1 ) {
  ...
  if ( !istext )			/* one-line func call for gifimage */
   gifimage = makegif(&nbytes,ncols,nrows,pixels,colortable,bgindex);
 ...
 * and add checkerboard tiling -- almost as easy
 * ----------------------------------------------------------- */
if ( testnum == 2 ) {
  /* --- set larger checkerboard colortable if necessary --- */
  int	checktable[999]= { /* larger gif rgb colortable for checkerboard */
  ...
  int	*pcolortable = (ischecker==0?colortable:checktable);
  /* --- initialize gif as before (as for test#1) --- */
  if( (gs=newgif(&gifimage,ncols,nrows,pcolortable,bgindex))/*init gifsave89*/
  ==  NULL ) goto end_of_job;		/* quit if failed */
  ...
  if (istext) plaintxtgif(gs,-1,-1, -1,-1, bgindex,1, text); /*overlay text*/
  /* --- now render frames comprising the animation --- */
  for ( iframe=0; iframe<nframes; iframe++ ) { /* render each frame */
    if ( iframe > 0 ) free((void *)pixels); /* first free previous frame */
    ...
    if ( ischecker != 0 ) {		/* checkerboard fg or bg pixels */
      int nchk=(ischecker<0?16008:8);
      int noff= (ischecker<0?-2*iframe:-2*(1000*iframe+iframe));
      pixels = pixcheck(ncols,nrows,pixels, nchk,noff,ncolors,
      (ischecker>0?fgindex:bgindex),(-99),(ischecker>0?-99:bgindex));
      /*(ischecker>0?fgindex:bgindex));*/ }
...
/* --- cleanup --- */
if ( pixels   != NULL ) free((void *)pixels);   /* free allocated pixels */
if ( gifimage != NULL ) free((void *)gifimage); /* free allocated image */
/* ----------------------------------------------------------------------- */
end_of_job:
/* ----------------------------------------------------------------------- */
  return ( 0 );				/* bye-bye */
...
||   nbytes < 1 ) goto end_of_job;	/* or no output requested */
...
if ( file != NULL )			/* have file arg */
 if ( *file != '\000' )			/* file arg not an empty string */
  ...
  == (FILE *)NULL ) fileptr = stdout;	/* revert to stdout if fopen fails */
/* -------------------------------------------------------------------------
...
if ( fileptr!=NULL && fileptr!=stdout ) fclose(fileptr); /* close file */
/* --- back to caller --- */
end_of_job:
